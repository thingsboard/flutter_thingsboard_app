...
class TbOAuth2Client implements IOAuth2Client {
  ...
  Future<TbOAuth2AuthenticateResult> authenticate(String oauth2Url) async {
    ...
    final path = await getIt<IEndpointService>().getEndpoint();
    Uri url = Uri.parse(path).replace(path: oauth2Url);
    ...
    _logger.debug('TbOAuth2Client::authenticate() result url -> $resultUri');
